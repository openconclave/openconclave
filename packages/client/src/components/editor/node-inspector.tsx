    ...
    <>
      ...
      {engine === "openai" && (
        <>
          <Field label="Provider">
            {providers.length === 0 ? (
              <p className="text-xs text-warning">
                No providers configured. <a href="/settings" className="underline text-primary hover:text-primary/80">Add one in Settings</a>
              </p>
            ) : (
              <select value={config.providerId ?? ""} onChange={(e) => update({ providerId: e.target.value, openaiModel: undefined })} className={INPUT_CLASS}>
                {providers.map((p) => (
          ...
          {selectedProvider && (
            <Field label="Model">
              {loadingModels ? (
                ...
                <select value={config.openaiModel ?? providerModels[0]} onChange={(e) => update({ openaiModel: e.target.value })} className={INPUT_CLASS}>
          ...
          <p className="text-[10px] text-muted-foreground px-1">
            {selectedProvider?.apiType === "responses" ? "Using OpenAI Responses API (with reasoning)" : "Using Chat Completions API"}
          </p>
        </>
      )}
